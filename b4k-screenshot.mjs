...
async function run() {
  ...
  console.log(`${ok} screenshots captured, ${fail} failed`);

  // Print stateful screens that need manual capture
  console.log('\n⚠️  STATEFUL SCREENS — need manual browser interaction:');
  console.log('  03 MP_10-13 Map POI selected  — click any map marker');
  console.log('  04 MP_20-23 Map plan active   — load a plan, then screenshot /map');
  console.log('  05 MP_30-35 Map AI overlay    — open AI assistant on /map');
  console.log('  16 PR_20-22 Profile saved     — route missing (no screenshot possible)');
  console.log('  21 AG_01-04 Auth gate         — trigger from any gated action (sign-in button)');
}
