...
export default function Page() {
  const t = useTranslations('routes.planManual');
  ...
  return (
    <main className="px-3.5 md:px-8 pt-7 pb-16 max-w-[1200px]" aria-label={t('title')}>
      <div className="flex items-center gap-1.5 text-f-xxs font-semibold tracking-[0.08em] uppercase text-muted mb-5">
        ...
        <span>›</span>
        <Link href="/plan" className="text-muted-2 hover:text-fg">{t('crumb.plan')}</Link>
        <span>›</span>
      ...
      >
        <Hand size={40} strokeWidth={2} className="text-muted-2 mb-4" />
