...
import type { Notification, NotificationType, NotificationsData } from '@/app/api/notifications/route'
...
export default function NotificationsClient() {
  ...
  const { data, isLoading, isError, mutate, hasMore, isLoadingMore, loadMore } = useNotifications()

  // mutate() here operates on the useSWRInfinite pages array, not a single
  // NotificationsData object — each page keeps its own notifications slice.
  function handleMarkRead(id: string) {
    fetch(`/api/notifications/${id}`, { method: 'PATCH' }).catch(() => {})
    mutate(
      (pages?: NotificationsData[]) => pages?.map(page => ({
        ...page,
        notifications: page.notifications.map(n => n.id === id ? { ...n, is_read: true } : n),
    ...
    >
      ...
      {!isLoading && !isError && data && (
        data.notifications.length === 0 ? (
          ...
          {hasMore && (
            <div className="flex justify-center mt-sp-4">
              <button
                onClick={loadMore}
