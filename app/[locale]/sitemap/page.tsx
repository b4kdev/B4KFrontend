...
export default function Page() {
  const t = useTranslations('routes.sitemap');
  ...
  return (
    <main className="px-3.5 md:px-8 pt-7 pb-16 max-w-[1200px]" aria-label={t('title')}>
      ...
      <h1 className="text-fg font-display text-f-display-tile mb-7">{t('title')}</h1>
      <div
        className="flex flex-col items-center justify-center text-center py-16 px-6 rounded-none"
        style={{ background: 'var(--bg-2)', border: '1px solid var(--bdr)' }}
      >
        <Info size={40} strokeWidth={2} className="text-fg opacity-[0.15] mb-4" />
